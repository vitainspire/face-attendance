...
# same tenant reuse a connection pool instead of opening a fresh one every time.
_tenant_sessionmakers = {}


def get_tenant_sessionmaker(supabase_url: str):
    if supabase_url not in _tenant_sessionmakers:
        tenant_engine = create_engine(supabase_url, pool_pre_ping=True, pool_recycle=1800)
